...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $columns            = array("id", "fullname", "email", "phone",  "status"); 
    $columns_list       = array("n.id", "n.fullname", "n.email", "n.phone", "n.status"); 
    ...
    //  SQL Srogusu
    $sql                = "SELECT n.id, n.fullname, n.email, n.phone, n.status FROM $table_name n WHERE n.trash != 1 ";
    ...
    if($s['num'] > 0)
    {
        ...
        foreach($datalist as $datas){
            ...
            foreach ($columns as $col) {
                ...
                {
                    if($datas[$col] == 1)
                        $new_data[] = '<span class="badge rounded-pill  badge-light-success">AKTİF</span>';
                    else
                        $new_data[] = '<span class="badge rounded-pill  badge-light-danger">PASİF</span>';
                }
                else
                    $new_data[] = $datas[$col];
            }
            ...
            $new_data[] = '
                <a href="'.$config->get('URL').'admin/'.$module_name.'/edit/'.$datas['id'].'/"
                    class="btn btn-warning">
                    <i data-feather="edit"></i>
                </a>
                <button data-id="'.$datas['id'].'" data-url="'.$config->get('URL').'admin/ajax/'.$module_name.'/delete.php"  class="btn btn-danger del_content_btn">
...
function total_data()
{
    ...
    $toplam = $db->getValue("SELECT COUNT(*) FROM ".$GLOBALS['table_name']." n WHERE n.trash != 1"); 
    return $toplam; 
}
...
function search_total_data($sql)
{
    ...
    $sql2 = "SELECT COUNT(*) FROM ".$GLOBALS['table_name']." n WHERE n.trash != 1 ".$sql;
